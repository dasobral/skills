#!/usr/bin/env python3
"""Register crypto policy in an external HMAC-authenticated approval ledger."""
...
def _validate_artifact(artifact: object) -> None:
    ...
    if schema_path is None:
        raise ValueError("crypto policy schema is unavailable")
    try:
        schema = json.loads(schema_path.read_text(encoding="utf-8"))
        Draft202012Validator.check_schema(schema)
        Draft202012Validator(schema).validate(artifact)
    except (OSError, UnicodeError, json.JSONDecodeError, SchemaError, ValidationError):
        raise ValueError("artifact fails the strict crypto policy schema") from None
...
def main() -> None:
    ...
    if (
        ...
        or set(artifact) != {"policy_version", "forbidden_primitives"}
        or not isinstance(artifact.get("policy_version"), str)
        or not re.fullmatch(
            r"[a-z0-9_.-]+@\d{4}-\d{2}-\d{2}", artifact["policy_version"]
        )
        or not isinstance(artifact.get("forbidden_primitives"), list)
        or not artifact["forbidden_primitives"]
        or not all(
            isinstance(item, str) and item.strip()
            for item in artifact["forbidden_primitives"]
        )
    ):
        parser.error("artifact is not a strict version-pinned crypto policy")
