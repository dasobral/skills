...
description: >
  ...
  parallel execution, artifact routing, failure handling, and synthesis.
  Bundled in the agent-platform plugin.
...
For parallel subtasks: run independent workers concurrently when the platform
supports it (see `references/platform-orchestration.md`). For sequential steps,
wait for artifacts before the next step.
