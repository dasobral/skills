...
SENSITIVE = re.compile(r"(?:^|_)(?:access_token|api_key|authorization|bearer|client_secret|credential|password|private_key|refresh_token|secret|seed|session_token|symmetric_key|token)(?:_|$)")
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if SENSITIVE.search(normalized):
                raise ValueError("credential-bearing field is forbidden")
...
def validate_contract(value: object, schema_name: str) -> None:
    if Draft202012Validator is None:
        print("evidence-gap: jsonschema==4.26.0 is unavailable; run the check-crypto-runtime skill for setup guidance", file=sys.stderr)
        raise SystemExit(3)
